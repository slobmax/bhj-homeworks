...
addInTrash.forEach(el => el.addEventListener('click', () => {
	...
	const findCart = cartInTrash.find((item => item.getAttribute('data-id') === cartId));    

	findCart ? findCart.querySelector('.cart__product-count').innerText = 
	Number(findCart.querySelector('.cart__product-count').innerText) + Number(cartCount) :
