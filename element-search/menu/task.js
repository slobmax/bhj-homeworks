...
for(i = 0; i < document.querySelectorAll('.menu__link').length; i++) {
	...
	currentElement.onclick = () => { 	
		if(currentElement.closest('li').querySelector('ul').className === 'menu menu_sub') {			
			currentElement.closest('li').querySelector('ul').className = 'menu menu_sub menu_active';
			return false;
		} else if(currentElement.closest('li').querySelector('ul').className === 'menu menu_sub menu_active') {
